...
exclude = {'USA/WA1/2020'};
%% distance based clustering

tree = phytreeread('orf8ko_noAlpha_tree.nwk');
% get all WA sequences
f = fopen('orf8ko_noAlpha_ko.tsv');
col = strsplit(fgets(f), '\t');
strain_id = find(ismember(col,'strain'));
orf8ko_id = length(col);
c=1;
id = cell(0,0); 
strain = cell(0,0); 
orf8ko = cell(0,0); 
while ~feof(f)
    ...
    orf8ko{c,1} = erase(col{orf8ko_id},newline);
    
    %if ~isempty(find(ismember(id{c,1}, exclude)))
    c=c+1;
end
fclose(f);
...
%%
is_ko = find(ismember(orf8ko,'Yes'));
...
%%
f = fopen('orf8ko_noAlpha_node_locations.tsv', 'w');
for i = 1 : length(nodenames)
    if ~contains(nodenames{i},'Branch')
        if strcmp(location{i},'orf8ko')
            fprintf(f, '%s\tORF8KO\n',nodenames{i});
        else
            fprintf(f, '%s\tNot ORF8KO\n',nodenames{i});
        end
    end
end
fclose(f);
...
for j = 1 : length(onlyKO)
    if length(location{j})==1 && location{j}=="orf8ko"
        onlyKO(j) = true;
...
f = fopen('orf8ko_noAlpha_clusters.tsv', 'w');
