...
rule download_metadata_s3:
    ...
    output:
        metadata = "data/genbank_meta.tsv.zst"
    params:
        address = "s3://nextstrain-data/files/ncov/open/metadata.tsv.zst"
    shell:
        """
        aws s3 cp {params.address} {output.metadata}
        """

rule download_aligned_s3:
    message: "Downloading metadata from {params.address} -> {output.metadata}"
    output:
        metadata = "data/genbank_aligned.fasta.zst"
    params:
        address = "s3://nextstrain-data/files/ncov/open/aligned.fasta.zst"
    shell:
        """
        aws s3 cp {params.address} {output.metadata}
        """
...
rule remove_blank:
    ...
    shell:
        """
        zstd -dc {input.metadata} | tsv-filter \
         -H --not-blank {params.notblank} | gzip > {output.metadata}
        """
...
rule include:
    ...
    conda: config["conda_environment"]
    log:
        "logs/include_{build_name}.txt",
    shell:
        """
        tsv-filter -H --str-eq inWA:True --ge coverage:0.95 {input.metadata} | cut -f 1 | sed -n '1!p' > {output.include}
        """
...
rule call_clusters:
    ...
    conda: config["conda_environment"]
    log:
        "logs/call_clusters_{build_name}.txt",
    shell:
        """
...
rule add_orf8ko_clusters:
    ...
    conda: config["conda_environment"]
    log:
        "logs/add_clusters_{build_name}.txt",
    shell:
        """
